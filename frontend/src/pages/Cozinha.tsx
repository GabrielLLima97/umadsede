...
export default function Cozinha() {
  ...
  const carregar = async () => {
    const [orders, items] = await Promise.all([
      api.get("/orders/?limit=500&ordering=created_at"),
      api.get("/items/?all=1"),
    ]);
    ...
    (Array.isArray(itemArr) ? itemArr : []).forEach((it: any) => {
      map[it.id] = { categoria: it.categoria };
    });
    setItemsMap(map);
  };
  ...
  const update = (id: number, status: string) =>
    api.patch(`/orders/${id}/status/`, { status }).then(carregar);
  ...
  const categoriaDoItem = (item: any) =>
    itemsMap?.[item.item]?.categoria || item.categoria || "Outros";
  ...
  return (
    <div className="flex flex-col gap-4 w-full">
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        {/* Coluna Pago / A preparar */}
        <div>
          <div className="mb-2 flex items-center justify-between rounded-2xl border border-sky-200 bg-sky-50 px-4 py-3 text-sky-700">
            <div className="font-black">Pago / A preparar</div>
            <span className="rounded-full bg-white/70 px-2 py-1 text-xs font-black">
              {String(col("pago").length + col("a preparar").length)}
            </span>
          </div>
        ...
        {/* Coluna Em produção */}
        <div>
          <div className="mb-2 flex items-center justify-between rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-amber-700">
            <div className="font-black">Em produção</div>
            <span className="rounded-full bg-white/70 px-2 py-1 text-xs font-black">
              {String(col("em produção").length)}
            </span>
          </div>
          <div className="flex flex-col gap-3">
        ...
        {/* Coluna Pronto */}
        <div>
          <div className="mb-2 flex items-center justify-between rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-emerald-700">
            <div className="font-black">Pronto</div>
            <span className="rounded-full bg-white
                        <span className="rounded-full bg-white/70 px-2 py-1 text-xs font-black">
              {String(col("pronto").length)}
            </span>
          </div>
          <div className="flex flex-col gap-3">
